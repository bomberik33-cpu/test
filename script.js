...
(() => {
    ...
    const players = await Promise.all(
      rosterRaw.map(async (m) => {
        ...
        try {
          ...
          return m; // fallback do skr�conych danych z rostera
        }
      })
    );
  ...
  // (leagueTeamId) niz gl�wne ID klubowe (faceitTeamId) � dlatego por�wnujemy
  ...
  async function loadMatches() {
    ...
    setStatus(upcomingEl, "Szukam najblizszych mecz�w�");
    setStatus(resultsEl, "Szukam ostatnich wynik�w�");
    ...
    for (const comp of competitions) {
      for (const kind of ["championships", "tournaments"]) {
        try {
          ...
          /* ta sciezka moze nie pasowac do danego ID � pr�bujemy dalej */
        }
      }
    }
  ...
  function renderUpcoming(list) {
    ...
    if (!list.length) {
      setStatus(el, "Brak zaplanowanych mecz�w w tej chwili.", "empty");
      return;
    }
    el.innerHTML = list
      .slice(0, 8)
  ...
  function renderResults(list) {
    ...
    if (!list.length) {
      setStatus(el, "Brak rozegranych mecz�w do pokazania.", "empty");
      return;
    }
    el.innerHTML = list
      .slice(0, 8)
      .map((m) => {
        ...
        <li class="match-row match-row--result">
          <a class="match-row__link" href="${matchUrl}" target="_blank" rel="noopener" aria-label="Szczeg�ly meczu SZPONT vs ${opp.name || "Przeciwnik"}"></a>
  ...
  async function loadLeagueBalance() {
    ...
    if (elos.length) {
      ...
      stats.push({ label: "Średnie ELO składu", value: avg });
    }

    // Stały panel Bilans z wartością 0:0
    stats.push({ label: "Bilans", value: "0:0" });
  ...
  function handleGlobalError(e, selector) {
    ...
    if (e && e.message === "MISSING_WORKER") {
      ...
      setStatus(el, "Nie udalo sie pobrac danych z FACEIT. Spr�buj ponownie za chwile.", "empty");
    }
    console.error(e);
  }
